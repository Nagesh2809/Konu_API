...
# python index.py
# http://127.0.0.1:5000/available_properties?location=kokapet
...
def available_properties():
    try:
        ...
        filtered_df = df[df['address'].str.contains(location, na=False)].copy()

        if filtered_df.empty:
            return jsonify({"message": f"No properties available in or near {location}"}), 404
...
def budget_properties():
    try:
        ...
        # df['new price'] = pd.to_numeric(df['new price'], errors='coerce')  # final_price
        ...
        filtered_df = df[(df['address'].str.lower() == locality) & (df['final_price'] <= budget)]
...
def project_price():
    try:
        ...
        filtered_df = df[df['project name'].str.strip().str.lower() == project_name]
